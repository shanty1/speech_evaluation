...
from model import EncoderRNN, DecoderRNN, DecoderRNN_SAME_INPUT
...
def main(args):
    ...
    decoder = DecoderRNN_SAME_INPUT(args.embed_size+Z_DIM, args.hidden_size, len(vocab), vocab('<pad>'), args.num_layers).to(device)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, ((audio,audio_len), (comment,comment_len)) in enumerate(data_loader):
            ...
            targets = pad_packed_sequence(pack_padded_sequence(batch_comment, comment_len_list, batch_first=True, enforce_sorted=False),batch_first=True)[0]
        ...
        if (epoch+1) % args.save_step == 0:
            torch.save(decoder.state_dict(), os.path.join(
                args.model_path, 'same-input-decoder-{}-{}.ckpt'.format(epoch+1, i+1)))
            torch.save(encoder.state_dict(), os.path.join(
                args.model_path, 'same-input-encoder-{}-{}.ckpt'.format(epoch+1, i+1)))
